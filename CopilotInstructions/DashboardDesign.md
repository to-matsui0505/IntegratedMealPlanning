...
## 対象範囲
- 食材の分類別在庫を円グラフで表示
- 最近の在庫増減をアクティビティ（履歴）として表示（最大5件、「もっと見る」リンク付き）
- 食材の在庫をリスト表示（品名・数量など）（最大5件、「もっと見る」リンク付き）
...
1. 円グラフ表示
   ...
   - 円グラフは水平バー形式で視覚化し、各カテゴリを色分け表示
   - 凡例で詳細を表示（カテゴリ名、品目数、割合）
2. 最近のアクティビティ
   - 項目: timestamp, type(add/use/discard/edit), itemName, qty, user
   - 初期表示は最新5件まで
   - 「もっと見る」リンクで全アクティビティ一覧画面へ遷移
3. 在庫リスト
   - 各行: 品名、数量、単位、カテゴリ
   - 初期表示は最新5件まで
   - 「もっと見る」リンクで全在庫一覧画面へ遷移
4. 操作ボタン（固定・オーバーレイ）
